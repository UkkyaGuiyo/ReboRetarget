...
Status: **in progress; Phase 2A through Phase 2D and Phase 2E offline preparation are complete; one Phase 2E live attempt is UNVERIFIED**
...
- Phase 2E offline safety preparation defines the natural Safe Point and adds a capacity-one latest-pose state primitive with strict receive/source ordering, stale/disconnect invalidation, and watermark-preserving rearm. A research-only bounded runner and synthetic SDK coverage now exercise the intended value path. One authorized 20.015-second official-SDK connection opened and closed safely but returned zero Pose callbacks, so the Live adapter/value/latency gate is `UNVERIFIED`, not PASS.
...
- Before another live attempt, require the user to confirm visibly updating action-calibrated ReboCap Pose, then require the accepted safety procedure and a new explicit authorization at an already-present natural Safe Point. Begin single-client-first; multi-client behavior remains separately opt-in and unverified. Send no VRChat OSC.
