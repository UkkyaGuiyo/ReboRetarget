...
## 2026-09-05 — Return-window false PASS and offline arm geometry

The user approved return-gate repair, arm length/balance and composite-morphology tests in that order, without Live access. One synthetic right cue used a stable neutral baseline, held root X=+0.3m and a return window alternating X=-0.3/+0.3m. Its mean returned to zero while every return sample remained displaced. The old analyzer reported PASS. Existing tests covered noisy baseline/held windows and a fixed non-return, but not this mean-cancelling return oscillation.

The repair checks existing return-window dispersion against the existing held/return allowance, also in yaw and relevant local/global joint branches. Large mean-cancelling return oscillation becomes UNVERIFIED; stable/small-noise returns and clear wrong-side FAIL remain supported. No aggregate fields, threshold relaxation, SDK lifecycle or external action were added. The synthetic reproduction diagnoses an analyzer omission, not a sensor or historical Live crash cause.

For arm geometry, reuse the accepted D-022 fixed-total leg semantics, confirmed hierarchy/rest-vector FK and existing anchor definitions rather than importing a solver. Prior official [VRChat rigging guidance](https://docs.vrchat.com/docs/full-body-tracking) reviewed on this date motivates proportion-sensitive validation but does not prescribe our control formula. D-034 records the independent project formula. Shoulder/hand dimensions and controller routing remain separate; no external source or dependency was copied. `tests/test_arm_morphology.py` provides analytic straight/bent-arm expectations, scale/share, malformed-number and forearm-only output-boundary cases. Test outcomes and the current validation boundary are indexed in `CURRENT_STATE.md`; no new Live or performance result is claimed.

`tests/test_composite_morphology.py` adds two independently calculated neutral eight-anchor/wrist layouts, two short asymmetric compound poses with all segment lengths and authored local rotations preserved, and +90-degree yaw/translation with q/-q through sixteen memory-only OSC messages. These are synthetic target geometries, not real avatar measurements or claims of hand fitting. Example: reference arm 0.28+0.25m, scale 1.2 and balance +0.1 gives upper arm 0.3996m and forearm 0.2364m (total 0.636m), excluding shoulder and hand.

Verification: the same **128 related tests PASS on Python 3.10, 3.11 and 3.13**. This comprises 101 tests across arm morphology/FK/adapter/replay/anchors/OSC/Quaternion normalization, 15 controlled analysis tests, 9 controlled-session tests and 3 composite tests. Runs were short groups, sequential across interpreters, not performance measurements. Reproduction and new cases used only synthetic RAM values. Scope Guard ACCEPT; no unnecessary implementation found. A new full-suite/watchdog/benchmark run is deferred until after play; the previous 219-test full-suite result does not certify this changed checkout.
