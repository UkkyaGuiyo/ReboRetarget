...
Phase 0.5, the read-only portion of Phase 1, a limited Phase 1.5 observation, the Phase 2A/2B/2C/2D pure-offline gates, and Phase 2E single-client Live value-path safety are complete. The output boundary still ends at memory-only OSC representation/codec. The current combined suite passes **219 tests on Python 3.10, 3.11 and 3.13** (140 at the Phase 2E checkpoint). Phase 2E PASS does not prove known-motion axes, physical tracking latency, real-avatar quality, product cadence, active-VR coexistence or multi-client safety. The earlier VRChat crash and the two unsuccessful historical Phase 2E attempts retain unresolved causes.
...
- **WAITING_FOR_USER:** Live work is paused for VRC play. Renewed readiness and a fresh Safe Point are required; the corrected four-stage wrapper still needs a representative offline timing check after play, not a repeated full functional suite without code changes.
...
After play, run the existing silent H benchmark on the corrected four-stage wrapper under controlled, non-competing conditions, reporting actual configuration, complete 60/20/20 windows, whole-run time and unchanged strict pure p99 <10ms. No heavy measurement during play and no automatic Live follow-on. Only after the remaining gate and renewed user readiness may a same-purpose controlled cue be considered with a fresh Safe Point. Scheduled guidance is not human acknowledgement or proof of audibility. Standard recording/coexistence remains a separate proposed gate in `PLAYTIME_SAMPLING_ASSESSMENT.md`.
