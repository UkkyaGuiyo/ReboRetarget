...
Phase 0.5, the read-only portion of Phase 1, a limited Phase 1.5 observation, the Phase 2A/2B/2C/2D pure-offline gates, and Phase 2E single-client Live value-path safety are complete. The output boundary still ends at memory-only OSC representation/codec. The current combined suite passes **216 tests on Python 3.10, 3.11 and 3.13** (140 at the Phase 2E checkpoint). Phase 2E PASS does not prove known-motion axes, physical tracking latency, real-avatar quality, product cadence, active-VR coexistence or multi-client safety. The earlier VRChat crash and the two unsuccessful historical Phase 2E attempts retain unresolved causes.
...
- **NEXT / WAITING_FOR_USER:** Offline recovery gates passed. No further Live connection/body cue without renewed explicit user readiness and a fresh Safe Point.
...
Only after the user explicitly reports being ready again may one same-purpose controlled cue be considered under a fresh Safe Point using the offline-validated countdown wrapper. Scheduled guidance is not human acknowledgement; audibility and actual motion remain separate evidence. Historical Live 20.05ms causality remains unconfirmed, and no OSC/VR application operation is authorized. Virtual Desktop and ReboCap settings/calibration remain protected. Do not request repeated movement while the user is away or start another Live connection autonomously.
