...
On 2026-09-05, one authorized 20.015-second single-client run opened and closed the official SDK connection successfully while ReboCap stayed alive, but delivered zero Pose callbacks. Consequently the live delta -> canonical -> latest -> FK -> anchors -> memory-only OSC value path and its latency remain `UNVERIFIED`. The research probe and synthetic tests establish the validation and lifecycle behavior only; they do not turn missing Live input into a PASS. Any retry requires a new authorization after the user confirms that the action-calibrated live skeleton/Pose is visibly updating.
