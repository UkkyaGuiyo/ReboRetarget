...
Current operational override: Live is paused for VRC play. The four-stage wrapper has three-version offline functional evidence, but no new representative timing measurement or real-audio proof. No extra SDK receiver or recording is authorized by the sampling investigation. See `PLAYTIME_SAMPLING_ASSESSMENT.md` for the unresolved standard-recording/coexistence boundary and `PHASE_2F_A_REPORT.md` for the corrected-wrapper results.
